...
namespace DigitalRuby.LaserSword
{
    public class LaserSwordScript : MonoBehaviour
    {
        ...
        private States statesScript;

        private bool mode; // true - lightsaber * false - the force
        ...
        private void Start()
        {
            ...
            statesScript = gameObject.GetComponentInParent<States>();
            BladeEnd.transform.position = BladeStart.transform.position;
        }

        private void Update()
        {
            if (OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) /*&& (OVRInput.Get(OVRInput.Button.Left) || (OVRInput.Get(OVRInput.Button.Right)))*/)
            {
                statesScript.changeCurrentState();
                Deactivate();
            }

            if (statesScript.getCurrentState().Equals("SWORD"))
            {
                if (OVRInput.Get(OVRInput.Button.Down) || Input.GetButtonDown("down"))
                {
                    Deactivate();
                }

                if (OVRInput.Get(OVRInput.Button.Up) || Input.GetButtonDown("up"))
                {
                    Activate();
                }
            }
        ...
        public bool TurnOn(bool value)
        {
            ...
            if (value)
            {
                bladeDir = 1.0f;
                state = 3;
                AudioSource.PlayOneShot(StartSound, 0.6f);
            ...
            else
            {
                bladeDir = -1.0f;
                state = 2;
                AudioSource.PlayOneShot(StopSound, 0.6f);
